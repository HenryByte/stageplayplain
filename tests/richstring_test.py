...
class LowLevelRichStringTests(TestCase):
    def test_plain_string_has_one_single_segment(self) -> None:
        s = plain("hello")
...
class RichStringOperatorTests(TestCase):
    def test_repr(self) -> None:
        ...
        self.assertEqual(
            "(bold)('Hello') + (plain)(' there ') + (bold)('folks')", repr(s)
        )

    def test_repr_on_empty_string(self) -> None:
        self.assertEqual("empty_string", repr(empty_string))

    def test_unicode(self) -> None:
        s = bold("Hello") + plain(" there ") + bold("folks")
        self.assertEqual("Hello there folks", str(s))
    ...
    def test_concatenating_two_richstrings(self) -> None:
...
class StyleGeneratorTests(TestCase):
    def test_bold_function_creates_bold_richstring(self) -> None:
        self.assertEqual(RichString(Segment("a", (Bold,))), bold("a"))

    def test_adding_functions(self) -> None:
        self.assertEqual(RichString(Segment("a", (Bold, Italic))), (bold + italic)("a"))


class RichStringTests(TestCase):
    def test_plain_to_html(self) -> None:
    ...
    def test_to_html(self) -> None:
        ...
        self.assertEqual(
            "<strong>bold</strong> normal <em>italic</em><u>wonderline</u>", s.to_html()
        )


class ParseEmphasisTests(TestCase):
    def test_parse_without_emphasis(self) -> None:
        self.assertEqual(
            plain("Hello"),
    ...
    def test_parse_bold(self) -> None:
        self.assertEqual(parse_emphasis("**Hello**"), bold("Hello"))

    def test_parse_pre_and_postfix_and_bold(self) -> None:
        self.assertEqual(
            parse_emphasis("pre**Hello**post"),
            plain("pre") + bold("Hello") + plain("post"),
        )

    def test_parse_multiple_bold(self) -> None:
        self.assertEqual(
            parse_emphasis("x**Hello** **there**"),
            plain("x") + bold("Hello") + plain(" ") + bold("there"),
        )

    def test_parse_adjacent_bold(self) -> None:
        self.assertEqual(parse_emphasis("**123**456**"), bold("123") + plain("456**"))

    def test_italic(self) -> None:
        self.assertEqual(parse_emphasis("*Italian style*"), italic("Italian style"))
    ...
    def test_italic_and_bold(self) -> None:
        self.assertEqual(
            parse_emphasis("***really strong***"), (bold + italic)("really strong")
        )

    def test_additional_star(self) -> None:
        self.assertEqual(
            parse_emphasis("*foo* bar* baz"), italic("foo") + plain(" bar* baz")
        )

    def test_underline(self) -> None:
        self.assertEqual(parse_emphasis("_hello_"), underline("hello"))

    def test_bold_inside_underline(self) -> None:
        self.assertEqual(parse_emphasis("_**hello**_"), (bold + underline)("hello"))

    def test_overlapping_underscore_and_italic(self) -> None:
    ...
    def test_simplified_complicated(self) -> None:
        self.assertEqual(
            parse_emphasis("*italic* or even ***bold italic.***"),
            italic("italic") + plain(" or even ") + (bold + italic)("bold italic."),
        )

    def test_two_italic_should_not_create_one_long_italic_string(self) -> None:
        self.assertEqual(
            parse_emphasis("*first* *second*"),
            italic("first") + plain(" ") + italic("second"),
        )

    def test_two_bold_should_not_create_one_long_bold_string(self) -> None:
        self.assertEqual(
            parse_emphasis("**first** **second**"),
            bold("first") + plain(" ") + bold("second"),
        )

    def test_escaping_star_creates_a_literal_star(self) -> None:
        self.assertEqual(parse_emphasis(r"\*hello*"), plain("*hello*"))
